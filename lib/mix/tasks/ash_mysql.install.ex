if Code.ensure_loaded?(Igniter) do
  defmodule Mix.Tasks.AshMysql.Install do
    @moduledoc "Installs AshMysql. Should be run with `mix igniter.install ash_mysql`"
    @shortdoc @moduledoc
    ...
    def igniter(igniter) do
      ...
      igniter
      |> Igniter.Project.Formatter.import_dep(:ash_mysql)
      |> setup_aliases()
    ...
    defp setup_repo_module(igniter, otp_app, repo) do
      default_repo_contents =
        """
        use AshMysql.Repo, otp_app: #{inspect(otp_app)}
        """
        ...
        fn zipper ->
          case Igniter.Code.Module.move_to_use(zipper, Ecto.Repo) do
            {:ok, _} ->
              ...
              |> use_ash_mysql_instead_of_ecto()
              |> Sourceror.Zipper.top()
              |> remove_adapter_option()
              |> then(&{:ok, &1})

            _ ->
              case Igniter.Code.Module.move_to_use(zipper, AshMySql.Repo) do
                {:ok, _} ->
                  {:ok, zipper}
                ...
                _ ->
                  {:error,
                   """
                   Repo module #{inspect(repo)} existed, but was not an `Ecto.Repo` or an `AshMysql.Repo`.

                   Please rerun the ash_mysql installer with the `--repo` option to specify a repo.
                   """}
              end
          end
        end
    ...
    defp use_ash_mysql_instead_of_ecto(zipper) do
      with {:ok, zipper} <- Igniter.Code.Module.move_to_module_using(zipper, Ecto.Repo),
           ...
           {:ok, zipper} <-
             Igniter.Code.Function.update_nth_argument(zipper, 0, fn zipper ->
               {:ok, Igniter.Code.Common.replace_code(zipper, AshMysql.Repo)}
             end) do
        zipper
      else
        _ ->
    ...
    defp remove_adapter_option(zipper) do
      with {:ok, zipper} <- Igniter.Code.Module.move_to_module_using(zipper, AshMysql.Repo),
           {:ok, zipper} <- Igniter.Code.Module.move_to_use(zipper, AshMysql.Repo),
           {:ok, zipper} <-
  ...
  defmodule Mix.Tasks.AshMysql.Install do
    ...
    def run(_argv) do
      Mix.shell().error("""
      The task 'ash_mysql.install' requires igniter to be run.
