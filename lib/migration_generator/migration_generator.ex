defmodule AshMysql.MigrationGenerator do
  @moduledoc false
  ...
  alias AshMysql.MigrationGenerator.{Operation, Phase}
  ...
  def generate(domains, opts \\ []) do
    ...
    snapshots =
      ...
      |> Enum.filter(fn resource ->
        Ash.DataLayer.data_layer(resource) == AshMysql.DataLayer &&
          AshMysql.DataLayer.Info.migrate?(resource)
      end)
  ...
  def take_snapshots(domain, repo, only_resources \\ nil) do
    ...
    |> Enum.filter(fn resource ->
      Ash.DataLayer.data_layer(resource) == AshMysql.DataLayer &&
        AshMysql.DataLayer.Info.repo(resource) == repo &&
  ...
  defp add_custom_index_name(custom_index, table) do
    ...
    |> Map.put_new_lazy(:name, fn ->
      AshMysql.CustomIndex.name(table, %{fields: custom_index.fields})
    end)
    |> Map.update!(
      :name,
      &(&1 || AshMysql.CustomIndex.name(table, %{fields: custom_index.fields}))
    )
  end
  ...
  def get_snapshots(resource, all_resources) do
    Code.ensure_compiled!(AshMysql.DataLayer.Info.repo(resource))
    ...
    if AshMysql.DataLayer.Info.polymorphic?(resource) do
      ...
      |> Enum.map(fn relationship ->
        ...
        |> Map.update!(:identities, fn identities ->
          identity_index_names = AshMysql.DataLayer.Info.identity_index_names(resource)
              ...
              Map.put(attribute, :references, %{
                ...
                destination_attribute_default:
                  default(
                    source_attribute,
                    relationship.destination,
                    AshMysql.DataLayer.Info.repo(relationship.destination)
                  ),
                deferrable: false,
                destination_attribute_generated: source_attribute.generated?,
                multitenancy: multitenancy(relationship.source),
                table: AshMysql.DataLayer.Info.table(relationship.source),
                on_delete: AshMysql.DataLayer.Info.polymorphic_on_delete(relationship.source),
                on_update: AshMysql.DataLayer.Info.polymorphic_on_update(relationship.source),
                primary_key?: source_attribute.primary_key?,
                name:
                  AshMysql.DataLayer.Info.polymorphic_name(relationship.source) ||
      ...
      [do_snapshot(resource, AshMysql.DataLayer.Info.table(resource))]
    end
  end
  ...
  defp do_snapshot(resource, table) do
    snapshot = %{
      ...
      table: table || AshMysql.DataLayer.Info.table(resource),
      ...
      base_filter: AshMysql.DataLayer.Info.base_filter_sql(resource),
  ...
  defp custom_indexes(resource) do
    ...
    |> Enum.map(fn custom_index ->
      Map.take(custom_index, AshMysql.CustomIndex.fields())
    end)
  end

  defp custom_statements(resource) do
    resource
    |> AshMysql.DataLayer.Info.custom_statements()
    |> Enum.map(fn custom_statement ->
      Map.take(custom_statement, AshMysql.Statement.fields())
    end)
  end
          ...
          %{
            ...
            table:
              relationship.context[:data_layer][:table] ||
                AshMysql.DataLayer.Info.table(relationship.destination)
          }
        end
      end
    end)
  ...
  defp foreign_key?(relationship) do
    Ash.DataLayer.data_layer(relationship.source) == AshMysql.DataLayer &&
      AshMysql.DataLayer.Info.repo(relationship.source) ==
        AshMysql.DataLayer.Info.repo(relationship.destination)
  end
  ...
  defp identities(resource) do
    ...
    |> case do
      ...
      identities ->
        ...
        if base_filter && !AshMysql.DataLayer.Info.base_filter_sql(resource) do
          raise """
    ...
    |> Enum.reject(fn identity ->
      identity.name in AshMysql.DataLayer.Info.skip_unique_indexes(resource)
    end)
    ...
    |> Enum.map(fn identity ->
      Map.put(
        ...
        :index_name,
        identity_index_names[identity.name] ||
          "#{AshMysql.DataLayer.Info.table(resource)}_#{identity.name}_index"
      )
    end)
    |> Enum.map(&Map.put(&1, :base_filter, AshMysql.DataLayer.Info.base_filter_sql(resource)))
  end
  ...
  defp configured_default(resource, attribute) do
    AshMysql.DataLayer.Info.migration_defaults(resource)[attribute]
  end
